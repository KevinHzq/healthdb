...
#' # otherwise, error due to the relation would not be one-to-one
#' fetch_vars(df1,
#'   keys = c(id, year),
#'   linkage = list(
...
# size <- 30
# n <- 10
# df1 <- data.frame(id = sample(1:n, size = size, replace = TRUE),
#                   service_dt = sample(seq(ymd("2020-01-01"), ymd("2022-01-31"), by = 1),
#                                       size = size)) %>%
#   dplyr::mutate(year = lubridate::year(service_dt))
# df2 <- data.frame(id = rep(1:n, size/n), year = rep(2020:2022, each = n),
#                   status_1 = sample(0:1, size = size, replace = TRUE),
#                   status_2 = sample(0:1, size = size, replace = TRUE))
# df3 <- data.frame(id = 1:n, sex = sample(c("F", "M"), size = n, replace = TRUE))
# db2 <- dbplyr::tbl_memdb(df2)
