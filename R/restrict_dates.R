...
#' @md
#' @description
#' For each client or group, interpret if they have n records that are at least certain days apart AND within a specified time span. When identifying events/diseases from administrative data, definitions often require, e.g., n diagnoses that are at least some days apart within some years. This function is intended for such use and optimized to avoid looping through all n-size combinations of dates per client. See `vignette("if_date_logic")` for an explanation of the algorithm and how it is translated into SQL.
#'
#'
...
#' @param force_collect A logical for whether to download the data and use the local method when both `apart` and `within` are supplied. For remote tables only. The default (FALSE) runs everything on the database, which involves writing a temporary table and an overlap (non-equi) join; if the database does not permit these, set TRUE to download the data and compute locally instead (may be slow). The argument has no effect when only one of `apart`/`within` is supplied, as those cases always run on the database.
