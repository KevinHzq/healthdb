test_that("basic use works", {
  ...
  out <- compute_dur(start, end, unit = unt) %>% round(2)
  expect_equal(out, round(ans, 2))
  # single end date
  ans_single <- purrr::map_dbl(start, ~ (as.Date("2020-10-27") - .)/365.25)
  out <- compute_dur(start, as.Date("2020-10-27"), unit = unt) %>% round(2)
  expect_equal(out, round(ans_single, 2))
  #also test grouped output
  out_fct <- compute_dur(start, end, lower_brks = c(0, 19, 25, 35, 45, 55))
...
test_that("date transform works", {
  ...
  out <- compute_dur(start, end, unit = unt) %>% round(1)
  # implicit trans chr to date
  expect_equal(out, rep(1,3))
  # trans with default ymd
  out <- compute_dur(start, end, unit = unt, trans = TRUE) %>% round(1)
  expect_equal(out, rep(1,3))
  # trans with supplied ydm
  out <- compute_dur(start, end, unit = unt, trans = TRUE, .transfn = lubridate::ydm) %>% round(1)
