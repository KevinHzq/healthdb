...
> One or more hospitalization with a substance use disorder diagnostic code, OR
> Two or more physician visits with a substance use disorder diagnostic code within one year.
...
2.  Bonus: remove clients with exclusion codes
    ...
    ```{r}
    result2 <- result1 %>% 
      exclude(excl = identify_rows(claim_db, starts_with("diagx_"), "in", "111"), by = "clnt_id")
    ```

3.  Restrict the number of records per client

    ```{r}
    result3 <- result2 %>% restrict_n(clnt_id = clnt_id, n_per_clnt = 2, count_by = dates)
    ```

4.  Restrict the temporal pattern of diagnoses

    \`restrict_dates\` also supports more complicated patterns like having n diagnoses at least i days apart within j years, but the "apart" feature requires relatively expensive computation and implemented for local data.frames only.
...
5.  Fetch variables from other tables by matching common keys
    ...
    result_df <- result_df %>% mutate(year = year(ymd(dates)))
...
To complete the definition, we need to repeat the process shown above with hospitalization data. Some studies may use more than a handful data sources to define their sample. We packed steps 1-4 in one function `define_case`, and provide tools to perform batch execution with different data and parameters to meet those needs.
...
sud_def <- build_def(
  ...
  fn_args = list(
    ...
    date_var = dates,
    within = c(365, NULL),
    keep = "first" # keeping the first diagnosis per client
  )
)
...
Let's look inside the fn_call list column. Two calls of `define_case` have been made with different parameters. The data arguments are left empty on purpose for re-usability. For example, theses calls can be executed again if there are newer data available, and you want to update your analysis.

```{r}
sud_def$fn_call
```

Executing the definition is simple. If verbose option is not turned off by `options(odcfun.verbose = FALSE)`, the output message will explain what has been done. You could append multiple `build_def` outputs together and execute all at once. Definition and source labels will be added to the result to identify outputs from different calls.
