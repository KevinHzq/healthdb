...
-   *Interactive functions* (e.g., `identify_row()`, `exclude()`, `fetch_var()`) based on filter and joins from dplyr with tweaks that fix SQL translation or add features that are not natively supported by SQL. They also work for local data.frames, and some use the 'data.table' package (`vignette("datatable-intro", package = "data.table")`) to speed up processing time for large data. These functions are not as flexible as `dplyr::filter()`, but they are general enough to be useful even outside health research.
...
-   *Miscellaneous functions* such as computing age (`compute_duration()`), collapsing records within a time range into one episode (`collapse_episode()`), and more (on-going effort). Most of these functions have built-in checks signalling when things might go wrong, e.g., missing values in calculated ages. These helpers are introduced in the data wrangling vignette (`vignette("wrangling")`).
...
4.  Restrict the temporal pattern of diagnoses
    ...
    `restrict_date()` supports more complicated patterns like having n diagnoses at least i days apart within j years (see `vignette("if_date_logic")` for how this works under the hood). Note that when SQL interpret order of dates, the result could be not deterministic if there were duplicate dates within client. Therefore, a unique row id (uid) has to be supplied to get consistent result.
...
If your case definition also needs an age restriction (e.g., only count records made when the client was within a certain age range), use `define_case_with_age()`. It takes all the same arguments as `define_case()` plus `birth_date`/`age` and `age_range`, applying the age filter before both the count (`n_per_clnt`) and temporal (`apart`/`within`) restrictions, so those are evaluated only on age-eligible records (e.g., "two or more visits while aged 18-65"). It can be supplied to `build_def(def_fn = ...)` just like `define_case()`.
...
Identifying the cases is usually just the beginning. The data wrangling vignette (`vignette("wrangling")`) introduces the helpers for what comes next: reporting sample sizes for your flowchart, computing age, decoding codes, scoring comorbidity, and reshaping records into episodes and periods.
