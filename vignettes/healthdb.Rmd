...
If your case definition also needs an age restriction (e.g., only count records made when the client was within a certain age range), use `define_case_with_age()`. It takes all the same arguments as `define_case()` plus `birth_date`/`age` and `age_range`, applying the age filter before the temporal (`apart`/`within`) restriction. It can be supplied to `build_def(def_fn = ...)` just like `define_case()`.
